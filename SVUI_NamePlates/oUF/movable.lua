...
do
	...
	getBackdrop = function(obj, isHeader)
		...
		local scale = CreateFrame('Button', nil, backdrop, BackdropTemplateMixin and "BackdropTemplate")
...
do
	...
	opt:SetScript("OnShow", function(self)
		...
		local scroll = CreateFrame("ScrollFrame", nil, self, BackdropTemplateMixin and "BackdropTemplate")
		scroll:SetPoint('TOPLEFT', subtitle, 'BOTTOMLEFT', 0, -8)
		scroll:SetPoint("BOTTOMRIGHT", 0, 4)

		local scrollchild = CreateFrame("Frame", nil, self, BackdropTemplateMixin and "BackdropTemplate")
		...
		local slider = CreateFrame("Slider", nil, scroll, BackdropTemplateMixin and "BackdropTemplate")
		...
		do
			...
			createEditBox = function(self)
				local editbox = CreateFrame('EditBox', nil, self, BackdropTemplateMixin and "BackdropTemplate")
				...
				if(style ~= '__INITIAL') then
					...
					for unit, points in next, styleData do
						if(not rows[numFrames]) then
							...
							local delete = CreateFrame("Button", nil, row, BackdropTemplateMixin and "BackdropTemplate")
							delete:SetWidth(16)
							delete:SetHeight(16)
		...
		local up = CreateFrame("Button", nil, slider, BackdropTemplateMixin and "BackdropTemplate")
		...
		local down = CreateFrame("Button", nil, slider, BackdropTemplateMixin and "BackdropTemplate")
