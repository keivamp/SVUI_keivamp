...
Report.OnEvent = function(self, event, ...)
	...
	-- else
		...
		-- self.text:SetText("", friendText, ((value - min) / (max - min) * 100))
	...
	if not name then
		...
		local myValue = ((value - min) / (max - min) * 100)
		local myValue2 = truncate(myValue,1)
		--local 
	
		self.text:SetText(txt.." "..myValue2.." %")
		
		
		local name, standing, min, max, value, factionID = GetWatchedFactionInfo()
		if factionID and C_Reputation.IsFactionParagon(factionID) then
		local currentValue, threshold, rewardQuestID, hasRewardPending, tooLowLevelForParagon = C_Reputation.GetFactionParagonInfo(factionID)
			local ParaMax = threshold
				local ParaCurrent = currentValue
				local doodley = 0
				for i = 1,999 do 
				
					if ParaCurrent > (ParaMax * doodley) then
						doodley = doodley + 1
					end
				end
					ParaCurrent = currentValue - (ParaMax * doodley) + threshold

			self.text:SetText("Paragon: "..ParaCurrent)

			--bar:SetStatusBarColor(0.95,0.85,0.55)
	...
	if not name then
		...
		local currentValue, threshold, rewardQuestID, hasRewardPending, tooLowLevelForParagon = C_Reputation.GetFactionParagonInfo(factionID)
			local ParaMax = threshold
				...
				bar:SetMinMaxValues(1, threshold)
				bar:SetValue(ParaCurrent)
			self.text:SetText("Paragon: "..ParaCurrent)

			--	self.text:SetText(name)
				--REPMaxText:SetText(threshold)
			--	RepCurrentText:SetText(ParaCurrent)
			--value = friendRep
			bar:SetStatusBarColor(0.95,0.85,0.55)
			--bar:SetMinMaxValues(min, max)
			--bar:SetValue(value)
