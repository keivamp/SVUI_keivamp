  	...
  	local function _update()
		...
		if(COUNT_GENERAL and (COUNT_GENERAL > 0)) then
		  wipe(FRIEND_DATA.General);
			...
			for i = 1, COUNT_GENERAL do
				local toonName, level, class, zoneName, isOnline, status, noteText = C_FriendList.GetFriendInfo(i)
		...
		if(COUNT_BNET and (COUNT_BNET > 0)) then
			...
			for i = 1, COUNT_BNET do
		        ...
		        if(toonID) then
		         -- 	hasFocus, toonName, client, realmName, realmID, faction, race, class, guild, zoneName, level, gameText, broadcastText, broadcastTime = C_BattleNet.GetAccountInfoByID(toonID);
		        else
		          --	hasFocus, toonName, client, realmName, realmID, faction, race, class, guild, zoneName, level, gameText, broadcastText, broadcastTime = C_BattleNet.GetAccountInfoByGUID(presenceID);
		        end
