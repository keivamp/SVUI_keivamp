...
local EMPTY_TEXTURE = [[Interface\AddOns\SVUI\assets\textures\EMPTY]];
local BLOOD_TEXTURE = [[Interface\AddOns\SVUI\assets\textures\EMPTY]];
...
local Reposition = function(self)
	...
	if(db.classbar.altComboPoints) then
		for i = 1, max do
			...
			if i == max then
                points[i].Icon:SetTexture(comboTextures[3])
            else
                points[i].Icon:SetTexture(comboTextures[random(1,2)])
            end
			if i==1 then
...
function MOD:CreateClassBar(playerFrame)
	...
	--	local coords = ICON_COORDS[random(2,4)]
		...
		blood:SetBlendMode("BLEND")

		SV.Animate:Sprite4(blood,0,0,true)
		cpoint.Blood = blood
